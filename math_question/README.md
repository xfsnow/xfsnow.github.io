...
### 任务一：优化 GeoGebra 画图命令生成

**目标**：根据初中数学几何题目描述，生成高质量的 GeoGebra XML 命令。

**核心技术点**：
1. 增强系统提示词，添加初中几何常见图形模板
2. 强调视图范围设置和图形美化原则
3. 构建命令验证系统，确保生成的 XML 符合 GeoGebra 规范
4. 收集高质量的 few-shot 示例引导 AI 生成

**预期效果**：
- AI 能够准确理解题目描述
- 生成的图形居中、美观、标注清晰
- 支持复杂几何构造（角平分线、垂直平分线、对称点等）
...
**目标**：将现有题库中的 webp/SVG 图片逐步替换为 GeoGebra 交互式画板。

**实施步骤**：
1. 扩展 `question.js` 数据结构，增加 `geogebra` 字段
2. 修改 `math_question.js`，支持渲染 GeoGebra 画板
3. 使用 ChatGG 批量生成 GeoGebra XML 命令
4. 手动校验和调整生成的命令
5. 实现平滑过渡：优先显示 GeoGebra 画板，无数据时回退到原始图片

**预期效果**：
- 学生可以在题目页面直接操作几何图形（缩放、拖拽）
- 解答中的辅助线可以逐步显示
- 所有图形使用统一的 GeoGebra 风格